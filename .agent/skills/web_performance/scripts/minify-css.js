...
const files = fs.readdirSync(targetDir).filter(f => f.endsWith('.css') && !f.endsWith('.min.css'));
...
files.forEach(file => {
    ...
    try {
        ...
        console.log(`✅ Minified ${file}: ${(oldSize / 1024).toFixed(1)}KB -> ${(newSize / 1024).toFixed(1)}KB (${savings}% saved)`);
    } catch (err) {
